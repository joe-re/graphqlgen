...
function renderNamespace(graphQLTypeObject, interfacesMap, unionsMap, typeToInputTypeAssociation, inputTypesMap, args) {
    return "    export namespace " + graphQLTypeObject.name + "Resolvers {\n\n    " + (args.defaultResolversEnabled
        ...
        : '') + "\n\n    " + renderInputTypeInterfaces(graphQLTypeObject, args.modelMap, interfacesMap, unionsMap, typeToInputTypeAssociation, inputTypesMap) + "\n\n    " + renderInputArgInterfaces(graphQLTypeObject, args.modelMap, interfacesMap, unionsMap) + "\n\n    " + renderResolverFunctionInterfaces(graphQLTypeObject, args.modelMap, interfacesMap, unionsMap, args.delegatedParentResolversEnabled, args.context) + "\n\n    " + renderResolverTypeInterface(graphQLTypeObject, args.modelMap, interfacesMap, unionsMap, args.delegatedParentResolversEnabled, args.context) + "\n\n    " + '' + "\n  }\n  ";
}
...
function renderResolverFunctionInterfaces(type, modelMap, interfacesMap, unionsMap, delegatedParentResolversEnabled, context) {
    return type.fields
        .map(function (field) {
        return "export type " + utils_1.upperFirst(field.name) + "Resolver = " + renderTypeResolver(field, type, modelMap, interfacesMap, unionsMap, delegatedParentResolversEnabled, context);
    })
        .join(os.EOL);
}
function renderResolverTypeInterface(type, modelMap, interfacesMap, unionsMap, delegatedParentResolversEnabled, context, interfaceName) {
    if (interfaceName === void 0) { interfaceName = 'Type'; }
    return "\n  export interface " + interfaceName + " {\n    " + type.fields
        .map(function (field) {
        return field.name + ": " + renderTypeResolver(field, type, modelMap, interfacesMap, unionsMap, delegatedParentResolversEnabled, context);
    })
        .join(os.EOL) + "\n      " + renderIsTypeOfFunctionInterface(type, modelMap, interfacesMap, unionsMap, context) + "\n  }\n  ";
}
var renderTypeResolver = function (field, type, modelMap, interfacesMap, unionsMap, delegatedParentResolversEnabled, context) {
    var parent;
    if (type.type.isInterface) {
    ...
    if (!delegatedParentResolversEnabled)
        return resolveFunc;
